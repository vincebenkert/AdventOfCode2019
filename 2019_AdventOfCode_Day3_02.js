...
for(let i=0; i<wireOneMap.length; i++){
	for(let z=0; z<wireTwoMap.length; z++){
		...
		let wireOneXYandStep = wireOneMap[i].split(",");
		let wireTwoXYandStep = wireTwoMap[z].split(",");
...
console.log("What is the fewest combined steps the wires must take to reach an intersection? " + leastSteps);
